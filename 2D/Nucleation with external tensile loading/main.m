...
StrucphiB0= ones(nx,ny,grainBs);
StrucphiB0(phiB==0)= 0;
StrucphiB= zeros(nx,ny,variants,grainBs);
for g= 1: grainBs
    for v= 1: variants
...
[tot_Cpq,hom_Cpq,het_Cpq,eigen,gomega,tot_Cpqrs,Spq] = greenmatrix_stiffnessmatrix_eigenstrain(grainBs,variants,phiB,psi,ang,e0,nx,ny,kx,ky,c11,c12,c44,k_pl);
...
%     [Sigmay2]= yieldstress(sumphiA2,sigmaA_pl,sigmaB_pl,kappaHP_pl,D_alpha_pl,G_pl,delta_Gm,Vmol,JCa_pl,JCb_pl,JCn_pl,JCm1_pl,JCm2_pl,T,T0,Tr,e1_pl,e2_pl,e3_pl);
    Sigmay2= 1e100* ones(nx, ny);                  % extremely large yield stress, so there is no plastic deformation
	
    [deldphiAk,E_elastic,e1_pl,e2_pl,e3_pl,output_s11,output_s22,output_s12] = solve_elasticity(phiA,StrucphiB,eigen,e1_pl,e2_pl,e3_pl,grainBs,variants,...
    ...
    if (istep<= nucleationstep)
        ...
        r1= rand(nx,ny,variants,grainBs); r2= rand(nx,ny,variants,grainBs);
        noise= -2* log(r1).* sin(2*pi*r2).*cos(2*pi*r2)*ampnoise;
        noisek= fft(fft(noise, [], 1), [], 2);
	...
	phiA= StrucphiB.* real(ifft(ifft(phiAk, [], 1), [], 2));
...
%         imagesc(sumphiAplot1); grid off; axis equal; axis([1 nx 1 ny]); colormap('jet'); colorbar;
...
%     if (mod(istep, nprint2)== 0)
		...
		filename4= ['ElasticEnergy_ttime_',num2str(ttime,'%6.4f'),'s.mat']; save(filename4, 'E_elastic');

    end
	
    maxsumphiAplot= max(sumphiAplot1,[],'all'); 	
	phiAplot= zeros(nx, ny, variants, grainBs); 
    sumphiAplot1= zeros(nx, ny); sumphiAplot2= zeros(nx, ny);	
	maxphiA= max(phiA,[],'all');
	if (maxsumphiAplot> 1| maxphiA< 1e-2| isinf(deldphiAk))	
        fileID= fopen('breakpoint.txt','w');
        fprintf(fileID,'maxsumphiAplot: %2d\r\n', maxsumphiAplot);
        fprintf(fileID,'maxphiA: %6.4f\r\n', maxphiA);
		break				
	end
