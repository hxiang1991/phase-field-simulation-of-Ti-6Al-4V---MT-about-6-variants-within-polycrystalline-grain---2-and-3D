function [deldphiAk, E_elastic, e1_pl,e2_pl,e3_pl, output_s11, output_s22, output_s12] = solve_elasticity(phiA,eigen,e1_pl,e2_pl,e3_pl,grainBs,variants,...
...
for g= 1: grainBs
    for v= 1: variants
        ...
        ei1= ei1+ eigen(1,1,v,g)* phiA(:,:,v,g); 
        ei2= ei2+ eigen(2,2,v,g)* phiA(:,:,v,g); 
        ei3= ei3+ eigen(1,2,v,g)* phiA(:,:,v,g); 
...
hom_e= zeros(3,1);
het_e= zeros(nx,ny,3); het_ek= zeros(nx,ny,3);
...
uk(:,:,1)= -1i*(gomega(:,:,1).*(sk(:,:,1).*kx+ sk(:,:,3).*ky)+ gomega(:,:,3).*(sk(:,:,3).*kx+ sk(:,:,2).*ky)); u(:,:,1,1)= real(ifftn(uk(:,:,1)));
uk(:,:,2)= -1i*(gomega(:,:,3).*(sk(:,:,1).*kx+ sk(:,:,3).*ky)+ gomega(:,:,2).*(sk(:,:,3).*kx+ sk(:,:,2).*ky)); u(:,:,2,1)= real(ifftn(uk(:,:,2)));

het_ek(:,:,1)= 1i* kx.* uk(:,:,1);                       
het_ek(:,:,2)= 1i* ky.* uk(:,:,2);                       
het_ek(:,:,3)= 0.5*1i* (kx.* uk(:,:,2)+ ky.* uk(:,:,1)); 

het_e(:,:,1)= real(ifftn(het_ek(:,:,1))); het_e(:,:,1)= het_e(:,:,1)- trapz(trapz(het_e(:,:,1)))/(nx*ny);
het_e(:,:,2)= real(ifftn(het_ek(:,:,2))); het_e(:,:,2)= het_e(:,:,2)- trapz(trapz(het_e(:,:,2)))/(nx*ny);
het_e(:,:,3)= real(ifftn(het_ek(:,:,3))); het_e(:,:,3)= het_e(:,:,3)- trapz(trapz(het_e(:,:,3)))/(nx*ny);
...
for iter= 1: niter
    ...
    T(:,:,1)= T0(:,:,1)- het_Cpq(:,:,1,1).* het_e(:,:,1)- het_Cpq(:,:,1,2).* het_e(:,:,2)- 2* het_Cpq(:,:,1,3).* het_e(:,:,3);
              
    T(:,:,2)= T0(:,:,2)- het_Cpq(:,:,2,1).* het_e(:,:,1)- het_Cpq(:,:,2,2).* het_e(:,:,2)- 2* het_Cpq(:,:,2,3).* het_e(:,:,3);
              
    T(:,:,3)= T0(:,:,3)- het_Cpq(:,:,3,1).* het_e(:,:,1)- het_Cpq(:,:,3,2).* het_e(:,:,2)- 2* het_Cpq(:,:,3,3).* het_e(:,:,3);
...
e11= hom_e(1)+ het_e(:,:,1)- ei1; 
e22= hom_e(2)+ het_e(:,:,2)- ei2;
e12= hom_e(3)+ het_e(:,:,3)- ei3;

e_devia= (het_e(:,:,1)+ het_e(:,:,2))/2;
...
for g= 1: grainBs
    for v= 1: variants
        ...
        deldphiA(:,:,v,g)= -(s11.* eigen(1,1,v,g)+ s22.* eigen(2,2,v,g)+ 2* s12.* eigen(1,2,v,g));
...
tot_e_shear(:,:,1,2)= het_e(:,:,3);
tot_e_shear(:,:,2,1)= het_e(:,:,3);
